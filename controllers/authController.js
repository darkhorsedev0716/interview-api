...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  sendMail({
    email: user.email,
    message,
    subject: "Your Account Activation Link for Hiiree!",
    user,
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (user.verified === false)
    return next(
      new AppError(
        `We are verifying your account! Please contact admin@hiiree.com to know the status.`,
        401
      )
    );
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  sendMail({
    ...
    message,
    subject:
      "Your Password reset link for Hiiree.com (will expire in 20 minutes)",
    user,
...
exports.createAdmin = catchAsync(async (req, res, next) => {
  console.log(req.body.adminPassword, process.env.ADMIN_CREATE_PASSWORD)
